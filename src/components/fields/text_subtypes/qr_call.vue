<template>
  <span>
    ...
    <span style="padding-left: 3px;" v-for="(p,idx) in splitted" :key="`qr-${idx}`">
      <a href="#"   @click.prevent="show_qr==p?(show_qr=false):(draw_qr(p))">{{ format_phone(p) }}</a><template v-if="idx!=splitted.length-1"> | </template>
    </span>
    ...
    <v-expand-transition>
      <div v-show="show_qr">
        ...
        <div class="call_buttons">
          <a :href="'tel:'+show_qr">
            <v-btn>
              <v-icon size="x-large" color="primary">mdi-phone</v-icon>
            </v-btn>
          </a>
